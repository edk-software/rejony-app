...
class AreaModel2018 implements CustomFormModelInterface, CompletenessCalculatorInterface
{
	private $translator;
	...
	public function constructForm(FormBuilderInterface $builder)
	{
		...
		$builder->add('facebookProfile', TextType::class, array('label' => 'Facebook profile', 'required' => false, 'attr' => ['help_text' => 'FacebookProfileHintText'], 'constraints' => [
			new Regex(array('pattern' => '/^[A-Za-zęĘąĄźŹżŻćĆłŁóÓńŃśŚ0-9\\.\\-]+$/', 'htmlPattern' => '^[A-Za-zęĘąĄźŹżŻćĆłŁóÓńŃśŚ0-9\\.\\-]+$', 'message' => $this->translator->trans('AreaFacebookProfileInvalidText')))
		]));
	}
	...
	public function createFormRenderer(): CustomFormRendererInterface
	{
        ...
        $r->group('Package information');
        $r->fields('packageCount','packageDetails');
		return $r;
	}
	...
	{
		$fieldCollection = ['parishName', 'parishAddress', 'parishPostal', 'parishCity', 'parishWebsite', 'responsiblePriest', 'responsibleCoordinator', 'contactPhone', 'areaWebsite', 'facebookProfile'];

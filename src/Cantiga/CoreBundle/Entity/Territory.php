...
class Territory implements IdentifiableInterface, InsertableEntityInterface, EditableEntityInterface, RemovableEntityInterface
{
    ...
    public static function fetchByName(Connection $conn, $territoryName, Project $project)
    {
        $data = $conn->fetchAssoc('SELECT * FROM `' . CoreTables::TERRITORY_TBL . '` WHERE `name` = :name AND `projectId` = :projectId', [':name' => $territoryName, ':projectId' => $project->getId()]);
        if (empty($data)) {
            return false;
        }
        $item = self::fromArray($data);

...
class AreaRequest implements IdentifiableInterface, InsertableEntityInterface, EditableEntityInterface, RemovableEntityInterface
{
    ...
    public function insert(Connection $conn)
    {
        ...
        $conn->insert(
            ...
            DataMappers::pick(
                $this,
                ['name', 'project', 'territory', 'requestor', 'createdAt', 'lastUpdatedAt', 'lat', 'lng', 'status'],
    ...
    public function update(Connection $conn)
    {
        ...
        if (null !== $this->postedMessage) {
            $this->commentNum = $conn->fetchColumn(
                'SELECT `commentNum` FROM `'.CoreTables::AREA_REQUEST_TBL.'` WHERE `id` = :id',
                [':id' => $this->id]
            );
            $conn->insert(
                CoreTables::AREA_REQUEST_COMMENT_TBL,
                [
                    'requestId' => $this->id,
        ...
        $cnt = $conn->update(
            ...
            DataMappers::pick(
                $this,
                ['name', 'status', 'territory', 'lastUpdatedAt', 'lat', 'lng', 'commentNum'],
                ['customData' => json_encode($this->customData)]
            ),
    ...
    {
        $stmt = $conn->prepare(
            'SELECT m.`createdAt`, m.`message`, u.`id` AS `user_id`, u.`name`, u.`avatar` FROM `'.CoreTables::AREA_REQUEST_COMMENT_TBL.'` m '

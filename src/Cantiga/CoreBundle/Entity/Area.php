...
class Area implements IdentifiableInterface, InsertableEntityInterface, EditableEntityInterface, HierarchicalInterface
{
    ...
    public static function prolongationArea(Connection $conn, MembershipRoleResolverInterface $resolver, $projectId, $areaId, $userId)
    {
        $newArea = self::fetchActive($conn, $areaId);
        $newArea->group = null;
        $newArea->project = Project::fetch($conn, $projectId);
        $newArea->territory = Territory::fetchByName($conn, $newArea->territory->getName(), $newArea->project);
        $newArea->insert($conn);
        $user = User::fetchByCriteria($conn, QueryClause::clause('u.`id` = :id', ':id', $userId));
        $newArea->setReporter($user);
        $newArea->getPlace()->joinMember($conn, $user, $resolver->getHighestRole('Area'), false, 0);
        return $newArea;
    }

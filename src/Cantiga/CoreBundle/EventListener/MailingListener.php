...
class MailingListener
{
    ...
    public function onAreaProlongation(AreaProlongationEvent $event)
    {
        $request = $event->getArea();
        $this->mailSender->send(
            CoreTexts::AREA_PROLONGATION_MAIL,
            $request->getReporter()->getEmail(),
            'notification about area request \'' . $request->getName() . '\' approval sent to \'' . $request->getReporter()->getEmail() . '\'',
            ['request' => $request, 'area' => $event->getArea()]
        );
    }
}

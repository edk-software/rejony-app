...
class UserAreaRequestController extends UserPageController
{
    ...
    /**
     * @Route("/continue/{step}/{projectId}/{areaId}", name="user_area_request_continue", requirements={"step": "(1|2)"}, defaults={"step": 1, "projectId": "select", "areaId": "select"},)
     */
    public function continueAction($step, $projectId, $areaId, Request $request)
    {
        $this->breadcrumbs()->entryLink($this->trans('Request a new area', [], 'pages'), $this->crudInfo->getInsertPage(), ['step' => $step]);

        switch ($step) {
            case 1:
                return $this->continueStep1($projectId, $areaId, $request);
            case 2:
                return $this->continueStep2($projectId, $areaId, $request);
        }
    }
    ...
    public function createStep1(Request $request)
    {
        ...
        $areas = $repository->getAvailableAreas($projects[0]['parentProjectId'], $this->getUser());
    ...
    public function continueStep1($projectId, $areaId, Request $request)
    {
        if (!ctype_digit($projectId)) {
            return $this->redirectToBeginning();
        }
        try {
            ...
            $form = $this->createForm(
                ContactDataForm::class, $item, [
                    'action' => $this->generateUrl('user_area_request_continue', ['step' => 1, 'projectId' => $projectId, 'areaId' => $areaId]),
                    'isUserRequestForm' => true,
                    'processingDataText' => $text->getContent()
                ]
            );
            ...
            if ($form->isValid()) {
                $this->getAreaRequestFlow()->persistContactData($request->getSession(), $item);
                return $this->redirectToRoute('user_area_request_continue', ['step' => 2, 'projectId' => $projectId, 'areaId' => $areaId]);
            }

            return $this->render($this->crudInfo->getTemplateLocation() . 'continue-step2.html.twig', [
                'form' => $form->createView(),
                'project' => $project
            ]);
        } catch (ModelException $ex) {
    ...
    public function continueStep2($projectId, $areaId, Request $request)
    {
        $session = $request->getSession();
        if (!ctype_digit($projectId) || !$this->getAreaRequestFlow()->isContinueCompleted($session)) {
            return $this->redirectToBeginning();
        }
        try {
            list($settings, $project, $formModel) = $this->loadEnvironment($projectId);

            $area = $this->getAreaRequestFlow()->prolongationArea($session, $project, $this->getUser(), $areaId);
            return $this->render($this->crudInfo->getTemplateLocation() . 'continue-step3.html.twig', ['project' => $project, 'slug' => $area->getSlug()]);
        } catch (ModelException $ex) {
            return $this->showPageWithError($this->trans($ex->getMessage()), $this->crudInfo->getIndexPage());
        }
    }

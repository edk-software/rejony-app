...
class ProjectAreaRequestController extends ProjectPageController
{
    ...
    public function initialize(Request $request, AuthorizationCheckerInterface $authChecker)
    {
        ...
        $this->breadcrumbs()
            ->workgroup('request')
            ->entryLink(
                $this->trans('Area requests', [], 'pages'),
    ...
    /**
     * @Route("/ajax/map", name="project_area_request_ajax_map")
     */
    public function ajaxMapAction(Request $request)
    {
        ...
        return new JsonResponse($routes->process($repository->listMapData($this->getTranslator())));
    }
    ...
    public function infoAction($id, Request $request, Membership $membership)
    {
        ...
        return $action->run(
            $this,
            $id,
            function (AreaRequest $item) use ($request) {
                $formModel = $this->extensionPointFromSettings(
                    CoreExtensions::AREA_REQUEST_FORM,
                    CoreSettings::AREA_REQUEST_FORM
                );
    ...
    public function approveAction($id, Request $request)
    {
        try {
            ...
            $question = new QuestionHelper(
                $this->trans(
                    'Do you want to approve the request \'0\'? This operation cannot be undone.',
                    [$item->getName()]
                )
            );
            $question->onSuccess(
                function () use ($repository, $item) {
    ...
    public function revokeAction($id, Request $request)
    {
        try {
            ...
            $question = new QuestionHelper(
                $this->trans(
                    'Do you really want to revoke the request \'0\'? This operation cannot be undone.',
                    [$item->getName()]
                )
            );
            $question->onSuccess(
